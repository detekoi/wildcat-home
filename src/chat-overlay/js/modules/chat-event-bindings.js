...
import { applyChromaKey } from './config-manager.js';

export function bindSettingsEvents(domRefs, managers) {
    const {
    ...
    document.querySelectorAll('input[name="chat-mode"]').forEach(input => {
        input.addEventListener('change', (e) => {
            if (e.target.checked) {
                if (switchChatMode) switchChatMode(e.target.value, false);
                if (updateModeSpecificSettingsVisibility) updateModeSpecificSettingsVisibility(e.target.value);
            }
        });
    });
    ...
    if (thirdPartyEmotesToggle) {
        thirdPartyEmotesToggle.addEventListener('change', () => {
            ...
            if (checked) {
                ...
                if (chatConnection && chatConnection.currentBroadcasterId) {
